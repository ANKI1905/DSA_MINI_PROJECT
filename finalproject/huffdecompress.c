...
void huffdecompress(int fdr, int fdw) {
	tree t;
	dtinit(&t);
	builttree(fdr, &t);
...
void builttree(int fdr, tree *t) {
/*build tree from traversal written as aheader to the file by reading the file*/
	tree temp;
	int i = 0;
	unsigned char ch;
	stack s;
...
void decode(int fdr, int fdw, tree *t){
/*read the file and get ci=odes and then get respective character from particular code*/
